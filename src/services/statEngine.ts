...
export function buildStatPrompt(stats: StatDefinition[] | undefined): string {
    ...
    // 게이지가 하나라도 있으면 '참고 있다'는 개념을 설명한다.
    // 이걸 빼면 모델이 게이지를 단순한 분노 수치로 읽고, 쌓이는 동안에도 감정을 그대로 드러낸다.
    const hasGauge = stats.some(s => s.kind === 'gauge');
    const gaugeGuide = hasGauge ? `
        게이지형은 '참고 있는 정도'다. 화를 참는 것만이 아니라, 하고 싶은 말·질투·욕망을
        삼키고 있는 상태 전부를 뜻한다.
        - 임계에 닿기 전까지 캐릭터는 그것을 겉으로 드러내지 않는다. 말끝이 흐려지거나
          시선이 따라가거나 손에 힘이 들어가는 정도로만 새어 나오게 하라.
        - 다른 수치(호감도·질투 등)가 아무리 높아도, 게이지가 남아 있는 한 캐릭터는 삼킨다.
          그 격차가 긴장을 만든다. 참지 않고 바로 드러내면 이 스탯은 의미가 없다.
        - 임계를 넘기면 시스템이 발현을 알리고 값을 되돌린다. 그 턴에는 눌러온 바로 그것이
          실제로 터져 나와야 한다.
` : '';
